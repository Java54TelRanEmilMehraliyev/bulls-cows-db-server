package test.java.telran.net.games.service;
...
import telran.net.games.service.BullsCowsGameRunner;
import telran.net.games.service.BullsCowsService;
import telran.net.games.service.BullsCowsServiceImpl;
...
class BullsCowsServiceTest {
	...
	void startGameTest() {
		...
		assertThrowsExactly(GameAlreadyStartedException.class, () -> bcService.startGame(gameIdNormalFlow));
	}
	...
	void notWinnerMoveTest() {
		...
		List<MoveData> moves = bcService.moveProcessing(twoBullsTwoCowsSequence, gameIdNormalFlow, username);
		MoveData moveDataExpected = new MoveData(twoBullsTwoCowsSequence, 2, 2);
	...
	@Test
	void winnerMoveTest() {
		List<MoveData> moves = bcService.moveProcessing(toBeGuessedSequence, gameIdNormalFlow, username);
		MoveData moveDataExpected = new MoveData(toBeGuessedSequence, N_DIGITS, 0);
	...
	void noGamerExceptionTest() {
		...
		assertThrowsExactly(NoGamerInGameException.class, () -> bcService.startGame(gameIdAltFlow));
